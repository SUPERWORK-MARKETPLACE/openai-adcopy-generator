...
`review.xlsx`의 검수 시트에서 각 항목의 상태값을 고른다:
`무수정 승인` / `수정 후 승인` / `사용 불가` / `부분 재생성` / `광고주 확인 필요`.
자동 검수 플래그(길이 초과·정책 확인 필요 등)가 있는 행은 검수상태가 기본값 `광고주 확인 필요`로 채워져 온다.
...
Context Hints 광고그룹당 5~10개 이상(질문형·상황형 우선) · 이미지 1:1(640×640~1200×1200px).
