...
- **정책 반영:** 정책·참고자료 시트의 공통 금지어를 읽어 SKU별 금지 표현과 병합해 최상위 `policy.banned_terms`를 채운다. **금지 표현**은 한 셀에 콤마(,)로 연결된 목록이다 — 항목별로 분리하고 앞뒤 공백을 트림한다. SKU별 **필수 포함 문구**는 그 SKU가 속한 각 `adgroup.required_phrases`에 넣는다 — 단, 필수 문구는 **콤마가 문장 일부인 단일 문장**일 수 있으니(예: "무료학습 조건과 대상, 체험 기간을 확인하세요") 기계적으로 쪼개지 말고 문장 단위로 판단한다. 필수 문구가 길면 copy에 담고 title은 자유 구성한다(title+copy 합산으로 검사).
