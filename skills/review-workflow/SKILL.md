...
- `validation_status`: **자동 검수 결과(읽기 전용)** — 형식·정책·사실성 검사 결과와 문제 유형, AI 검수 노트. 표준 플래그 어휘 8종: `의미 중복 후보`·`경고`·`광고주 확인 필요`·`길이 초과`·`길이 권장 초과`·`근거 확인 필요`·`정책 확인 필요`·`문장 자연성 확인 필요`. 운영자가 수정하지 않는다.
- `검수상태`: **운영자 판정 드롭다운** — 아래 5종. validation_status에 플래그 토큰이 포함된 행(기계 검사 `오류(...)` 병합 행 포함)은 review-out이 기본값 "광고주 확인 필요"를 미리 기입해 온다(운영자가 확인 후 변경) — 자동 `무수정 승인` 기입은 없다. finalize는 이 열만 본다(review-in JSON의 `review_status`).
...
| 광고주 확인 필요 | 제외하고 확인 대기 목록으로 보고 (플래그 행에 review-out이 기본 기입한 값 포함) |
...
- 자동 검수는 오류 후보 선별일 뿐 — 승인 권한은 운영자·광고주에게 있다. AI가 `검수상태`(운영자 판정)를 임의로 채우지 마라. 단, review-out CLI는 validation_status에 플래그 토큰이 있는 행에 한해 `검수상태` 기본값 "광고주 확인 필요"를 기입한다 — 자동 `무수정 승인` 기입은 어디에도 없다. `validation_status`(자동 검수 결과)에 "광고주 확인 필요"·"근거 확인 필요" 등 플래그를 표시하는 것은 AI 몫이다.
