...
- `validation_status`: **에이전트 자동 검수 결과(읽기 전용)** — 형식·정책·사실성 검사 결과와 문제 유형, AI 검수 노트. 표준 플래그 어휘 8종: `의미 중복 후보`·`경고`·`광고주 확인 필요`·`길이 초과`·`길이 권장 초과`·`근거 확인 필요`·`정책 확인 필요`·`문장 자연성 확인 필요`. 운영자가 수정하지 않는다. **`광고주 확인 필요` 플래그는 이 열 전용**이다(0715 요청 1).
- `검수상태`: **광고주 검수 결과 입력 드롭다운** — 아래 4종. **전 행 빈칸이 기본값**(0715 요청 1) — review-out은 어떤 값도 미리 기입하지 않는다. 광고주가 확인 후 직접 기입한다(운영자 대행 가능). finalize는 이 열만 본다(review-in JSON의 `review_status`).
- `review_comment`: 운영자·광고주 수정 의견·재생성 사유 기입란.

## 검수 상태값 (광고주가 review.xlsx `검수상태` 드롭다운에서 선택 — 4종)

| 상태 | finalize 처리 |
|---|---|
| 무수정 승인 | 그대로 최종 파일에 포함 |
| 수정 후 승인 | 시트에서 고친 title/copy/keywords를 반영해 포함 (재검증 필수) |
| 사용 불가 | 제외 |
| 부분 재생성 | review_comment를 반영해 재생성 → 재검증 → 새 review.xlsx로 재검수 요청 (최종 파일에 바로 넣지 않는다) |
| (빈칸) | 미검수 — 제외하고 미검수 목록으로 보고 (validation_status 플래그 행이면 플래그 내용도 함께 보고) |

`광고주 확인 필요`는 검수상태 값이 아니다 — 이 값이 검수상태 열에 들어오면 review-in이 Problems로 보고한다.

review.xlsx 요약 시트에 상태값별 1줄 설명이 포함된다(review-out 기입) — 검수자가 시트만 보고도 각 상태의 finalize 처리를 알 수 있다.

## finalize 절차 (순서 고정)

1. `adcopy review-in review.xlsx generated.json` — Problems가 있으면 **중단**하고 목록 보고 (원본과 매칭 안 되는 행·허용 외 검수상태 값 등). 광고주 판정은 결과 JSON의 `review_status` 필드로 들어온다.
   ...
   - **`flagged_unreviewed` 활용(0715):** 자동 검수 플래그가 있는데 검수상태가 빈칸(미검수)으로 남은 행 목록 — 미검수 보고에서 이 행들은 플래그 내용과 함께 표시한다(깨끗한 미검수 행과 구분).
...
- 자동 검수는 오류 후보 선별일 뿐 — 승인 권한은 운영자·광고주에게 있다. **`검수상태`(광고주 판정)는 누구도(AI·CLI 모두) 미리 채우지 않는다 — 전 행 빈칸 디폴트**(0715 요청 1). `validation_status`(자동 검수 결과)에 "광고주 확인 필요"·"근거 확인 필요" 등 플래그를 표시하는 것은 AI 몫이다.
